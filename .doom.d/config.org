...
#+begin_src emacs-lisp :tangle yes
(setq org-agenda-sorting-strategy '((agenda user-defined-up deadline-up priority-down todo-state-up scheduled-up effort-up habit-up)
...
(setq org-todo-keywords '((sequence "PROJ(p)" "TODO(t)" "LOOP(l!)" "HABT(h!)" "WAIT(w@/@)" "NEXT(n)" "IDEA(i)" "SOMEDAY(m)" "NOTE(o)" "|" "DONE(d!)" "KILL(k@)")))
