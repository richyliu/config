...
#+begin_src emacs-lisp :tangle yes
(setq org-agenda-sorting-strategy '((agenda user-defined-up category-keep habit-up scheduled-up deadline-up todo-state-up priority-down tag-up)
                                    (todo category-keep todo-state-up priority-down deadline-up ts-up)
...
(setq org-agenda-custom-commands '(("d" "Daily agenda and TODOs"
                                    ((todo "TODO" ((org-agenda-dim-blocked-tasks 'invisible)
                                                   (org-agenda-skip-function '(org-agenda-skip-entry-if 'scheduled))
                                                   (org-agenda-overriding-header "Unscheduled TODOs")))
                                     (todo "PROJ" ((org-agenda-dim-blocked-tasks nil)))
                                     ...
                                     (todo "PROJIDEA" ((org-agenda-dim-blocked-tasks 'invisible)))))
...
(setq org-todo-keywords '((sequence "TODO(t)" "SCHD(s)" "LOOP(l)" "PROJIDEA(r)" "PROJ(p)" "NEXT(n)" "WAIT(w)" "IDEA(i)" "SOMEDAY(m)" "|" "DONE(d@)" "KILL(k@)")))
...
(defun my/projectile-groups ()
  ...
  (cond
   ...
   ((and (string-equal default-directory org-directory)
         (member (buffer-name) '("inbox.org" "journal.org" "agenda_archive.org")))
...
#+end_src
** Lispy
Enable lispy when in janet-mode so that lispyville is also enabled
#+begin_src emacs-lisp :tangle yes
(add-hook 'janet-mode-hook #'lispy-mode)
#+end_src
** LSP
#+begin_src emacs-lisp :tangle yes
(after! lsp-mode
